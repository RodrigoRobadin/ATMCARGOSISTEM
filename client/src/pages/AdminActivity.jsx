...
export default function AdminActivity() {
    ...
    // Ranking por acciones + profit
    const actionsByUser = useMemo(() => {
        const g = groupBy(actsNow, (a) => a.user_id || 'desconocido');
        const rows = [];

        // 🔍 DEBUG: Ver todos los deals
        console.log('📊 Total deals:', deals.length);
        if (deals.length > 0) {
            console.log('📊 Primer deal:', deals[0]);
        }
        ...
        for (const [uid, list] of g.entries()) {
            ...
            // Operaciones: buscar por AMBOS campos posibles
            const ownDeals = deals.filter(
                (d) =>
                    d.advisor_user_id === uid || d.deal_advisor_user_id === uid
            );
            ...
            // 💰 Calcular profit total del usuario
            const totalProfit = ownDeals.reduce(
                (sum, d) => sum + (Number(d.value) || 0),
                0
            );

            // 🔍 DEBUG: Ver deals por usuario
            if (ownDeals.length > 0) {
                const userName = usersById.get(uid)?.name || `Usuario ${uid}`;
                console.log(`💰 ${userName}:`, {
                    deals: ownDeals.length,
                    values: ownDeals.map((d) => ({
                        ref: d.reference,
                        value: d.value,
                    })),
                    totalProfit,
            ...
            rows.push({
                ...
                ops,
                profit: totalProfit, // 💰 NUEVO
            });
        }

        rows.sort((a, b) => b.profit - a.profit); // 💰 Ordenar por profit
        return rows;
...
function Th({ children, onClick, active = false, dir = 'desc', right = false }) {
    return (
        <th
            className={`px-3 py-2 text-left ${right ? 'text-right' : ''
