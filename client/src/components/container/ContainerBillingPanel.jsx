...
const SUMMARY_STATUSES = ["pendiente", "facturado", "cobrado", "anulado"];
...
export default function ContainerBillingPanel({ dealId }) {
  const { user } = useAuth();
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [{ data: contractsData }, { data: billingData }] = await Promise.all([
        api.get(`/container/deals/${dealId}/contracts`),
        api.get(`/container/deals/${dealId}/billing`),
      ]);
      setContracts(Array.isArray(contractsData) ? contractsData : []);
      setRows(Array.isArray(billingData) ? billingData : []);
    } catch (err) {
      console.error("load container billing panel", err);
      setContracts([]);
      setRows([]);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border bg-white p-5">
        <div className="font-semibold">Facturacion mensual</div>
        ...
        {!isAdmin && (
          <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-600">
            Tu rol puede generar y seguir mensualidades, pero la factura fiscal la emite Administracion.
          </div>
        )}
      ...
      <div className="rounded-2xl border bg-white p-5">
        <div className="font-semibold">Generar mensualidad</div>
        <div className="text-sm text-slate-500 mt-1">
          Se toma el alquiler del contrato y genera el siguiente ciclo pendiente.
        </div>
        <div className="mt-4 flex flex-wrap gap-3">
          {contracts.map((contract) => (
            <button
              key={contract.id}
              type="button"
              className="px-3 py-2 rounded-xl border text-sm bg-white disabled:opacity-60"
              onClick={() => generateBilling(contract.id)}
              disabled={busyId === `generate-${contract.id}`}
            >
              {contract.contract_no || `Contrato #${contract.id}`} · Generar mensualidad
            </button>
          ))}
          {!contracts.length && (
            <div className="text-sm text-slate-500">No hay contratos disponibles para esta operacion.</div>
          )}
        </div>
      </div>
